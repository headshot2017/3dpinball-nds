...
void pb::keyup()
{
	...
	if (nds_input::MoveLeftPaddleUp())
	{
		MainTable->Message(1001, time_now);
	}

	if (nds_input::MoveRightPaddleUp())
	{
		MainTable->Message(1003, time_now);
	}

	if (nds_input::LaunchBallUp())
	{
		MainTable->Message(1005, time_now);
	}

	if (nds_input::NudgeRightUp())
	{
		nudge::un_nudge_right(0, nullptr);
	}
	else if (nds_input::NudgeLeftUp())
	{
		nudge::un_nudge_left(0, nullptr);
	}
	else if (nds_input::NudgeUpUp())
	{
...
void pb::keydown()
{
	...
	if (nds_input::MoveLeftPaddleDown())
	{
		MainTable->Message(1000, time_now);
	}

	if (nds_input::MoveRightPaddleDown())
	{
		MainTable->Message(1002, time_now);
	}

	if (nds_input::LaunchBallDown())
	{
		MainTable->Message(1004, time_now);
	}

	if (nds_input::NudgeRightDown())
	{
		if (!MainTable->TiltLockFlag)
			nudge::nudge_right();
	}

	if (nds_input::NudgeLeftDown())
	{
		if (!MainTable->TiltLockFlag)
			nudge::nudge_left();
	}

	if (nds_input::NudgeUpDown())
	{
		if (!MainTable->TiltLockFlag)
			nudge::nudge_up();
	}
